...
kimaki session search 'query'          # last 14 days
kimaki session search 'query' --days 0 # all time
kimaki session search 'query' --all
kimaki session read <session-id>
kimaki session read <session-id> --thinking
kimaki session read <session-id> --verbose
kimaki session read <session-id> --tool-input-max-chars 40
...
See [CI & Automation](../guides/ci-automation) for the full `kimaki send` flag reference and scheduling options. See [Debug sessions and report bugs](../guides/report-bugs.mdx) to export jsonl and file an issue.
