...
**Tool Attribution** — Shell commands started by a Discord-backed OpenCode session receive `KIMAKI_THREAD_ID` with the owning Discord thread ID. The value is resolved for each tool invocation, so concurrent sessions cannot overwrite each other's attribution. Manual OpenCode sessions omit it.
