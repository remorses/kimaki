...
- **Working directory changes** (for example after [`/new-worktree`](/docs/worktrees) moves the session into a worktree).
...
- Pair it with [the queue](/docs/queue): queue "update MEMORY.md with the key takeaways" to run when a session finishes.
