...
To protect specific folders, add `deny` or `ask` rules to your `opencode.json`. Your project config is loaded after Kimaki's defaults, so it wins:
...
<Aside>
<Note>
Worktree threads always deny the original checkout, with or without the flag. This keeps the agent from editing the main repo after the thread moved to a worktree.
</Note>
</Aside>
