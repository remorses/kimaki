...
kimaki session read ses_xxx --thinking --verbose > ./tmp/ses_xxx-full.md
kimaki --version
```
...
Each line is one raw OpenCode event. Read it before you write the issue.

```bash
# event type counts
jq -r '.type' ./tmp/ses_xxx.jsonl | sort | uniq -c

# lifecycle only
jq -r 'select(.type | startswith("session.execution.")) | [.created, .type, .data.sessionID, .data.executionID] | @tsv' ./tmp/ses_xxx.jsonl

# timestamps + types
jq -r '[.created, .type] | @tsv' ./tmp/ses_xxx.jsonl
```
...
**What the files show:**
- jsonl line N: `session.execution.started` at <created>
- jsonl line M: `session.execution.interrupted` then a footer message was posted
