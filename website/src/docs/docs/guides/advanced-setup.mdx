...
To control multiple machines, run **gateway mode** on each box and install the shared bot in the same Discord server. Agents should follow [Set up Kimaki from an agent](./agent-machine-setup.mdx).

1. Run `npx -y kimaki@latest --gateway` on each machine
2. Authorize the install URL in the same Discord server
3. Map projects with `kimaki project add` or `/add-project`
...
To reach the OpenCode server from your laptop (`opencode attach`, `opencode web`), start Kimaki with `--opencode-hostname 0.0.0.0`, a fixed `--opencode-port`, and `OPENCODE_SERVER_PASSWORD`. See [Expose the OpenCode server](../remote-access/opencode-server.mdx).
