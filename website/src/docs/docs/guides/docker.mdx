...
| `--opencode-hostname 0.0.0.0` | Bind the OpenCode server on every interface so you can `opencode attach` from another machine. Requires `OPENCODE_SERVER_PASSWORD`. See [OpenCode server](../remote-access/opencode-server.mdx). |
