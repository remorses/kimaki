...
Kimaki can also speak: when you ask for audio of some text, the agent can generate speech with `kimaki tts` and post it to the thread. See the [Commands reference](/docs/commands) for the `tts` and `upload-to-discord` CLI subcommands.
