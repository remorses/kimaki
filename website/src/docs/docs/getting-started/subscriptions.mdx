...
Rate-limited accounts go into a **cooldown** shared by every session on the machine, so nothing retries a subscription that is known to be out. A `429` honors `retry-after` with a five minute floor; a `402` (balance exhausted) waits six hours.
