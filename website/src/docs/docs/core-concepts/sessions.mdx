...
- **`/last-sessions`** — a quick cross-project list of what you were last working on. Titles prefixed **`btw:`** are [side-question forks](../features/btw.mdx). They answer a related question in parallel and are not duplicate sessions.
...
To export the session event stream and file a Kimaki bug, see [Debug sessions and report bugs](../guides/report-bugs.mdx).
...
Kimaki also shows context usage automatically in the thread at roughly 10% increments (prefixed `⻟`), and at the end of each assistant turn in a quoted footer like `> *kimakivoice ⋅ main ⋅ 2m 30s ⋅ 71% ⋅ claude-opus*`.
