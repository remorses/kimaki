...
Kimaki also shows context usage automatically in the thread at roughly 10% increments (prefixed `⬦`).
The agent ends a finished turn by pinging you with a short summary, which creates the Discord sidebar notification.
Start Kimaki with `--session-footers` if you also want the old metadata line like `kimakivoice ⋅ main ⋅ 2m 30s ⋅ 71% ⋅ claude-opus`.
