...
```diagram
   /resume <session>  ──▶ continue a past session (autocomplete lists recent ones)
   /new-session       ──▶ start a fresh session with a prompt, files, and agent
   /last-sessions     ──▶ list the 20 most recently active sessions across all projects
```
...
- **[`/btw`](/docs/btw)** — fork the whole context into a new thread to answer a side question in parallel.
...
- **[`/diff`](/docs/diff-viewer)** — share the current changes as a diff URL.
