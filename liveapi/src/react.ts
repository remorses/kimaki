...
import type { LiveServerMessage, FunctionCall, Part, CallableTool } from '@google/genai'
...
export function useAudioChat(options: UseAudioChatOptions): UseAudioChatResult {
  const { url, headers, model, onToolCall } = options
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch tools and token: ${response.statusText}`)
      }
      
      const { token, tools: toolsData }: ToolsAndTokenResponse = await response.json()
      
      // Create callable tools that execute via server API
      const callableTools: Array<CallableTool & { name: string }> = toolsData.map((toolDef) => ({
        name: toolDef.name,
        async tool() {
          return {
          ...
          for (const functionCall of functionCalls) {
            ...
            try {
              ...
              if (onToolCall) {
                ...
                pendingToolResults.current.delete(toolCall.toolCallId)
                
                // Let client optionally handle the tool call
                await Promise.resolve(onToolCall({ toolCall }))
                
                // Check if a result was added via addToolResult
                const pendingResult = pendingToolResults.current.get(toolCall.toolCallId)
                if (pendingResult) {
                  pendingToolResults.current.delete(toolCall.toolCallId)
                  parts.push({
                    functionResponse: {
