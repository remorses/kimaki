...
- `kimaki task edit <id> --background true|false` flips existing tasks in place, so a noisy recurring task can go quiet without recreating it. For thread tasks the stored user is also removed from the thread (best-effort) so it disappears from their sidebar immediately; `--background false --user` makes the task fully visible again. `kimaki task list` marks background tasks with `(bg)` next to the userId column.
