...
async function main() {
  ...
  if (!existingBot) {
    cancel(
      'No bot credentials found. Please run the main CLI first to set up your bot.',
    )
    process.exit(1)
  }
  ...
  try {
    ...
    console.log(
      'For instant updates during development, consider using guild-specific commands.',
    )
