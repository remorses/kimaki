    ...
    for (const project of projects) {
      ...
      try {
        ...
        if (sessionsResponse.data) {
          ...
          if (projectSessions.length > 0) {
            ...
            if (latestSession) {
              console.log(`     Latest: "${latestSession.title}" (${new Date(latestSession.time.updated).toLocaleString()})`)
            }
          }
        }
      } catch (e) {
      ...
      try {
        ...
        if (fileStatusResponse.data) {
          const modifiedCount = fileStatusResponse.data.filter(f => f.status === 'modified').length
