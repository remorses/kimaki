...
// ============================================================================
// Session Thinking Functions
// ============================================================================

/**
 * Get the thinking preference for a session.
 */
export async function getSessionThinking(
  sessionId: string,
): Promise<string | undefined> {
  const prisma = await getPrisma()
  const row = await prisma.session_thinking.findUnique({
    where: { session_id: sessionId },
  })
  return row?.thinking_value
}

/**
 * Set the thinking preference for a session.
 */
export async function setSessionThinking(
  ...
  const prisma = await getPrisma()
  await prisma.session_thinking.upsert({
    where: { session_id: sessionId },
    create: { session_id: sessionId, thinking_value: thinkingValue },
    update: { thinking_value: thinkingValue },
  })
}
