...
e2eTest('thread message queue ordering', () => {
  ...
  test('first prompt after cold opencode server start still streams text parts', async () => {
  ...
  test('text message during active session gets processed', async () => {
    ...
    const userBIndex = after.findIndex((m) => {
      return m.author.id === TEST_USER_ID && m.content.includes('beta')
    })
  ...
  test('two rapid text messages in thread — both processed in order', async () => {
    ...
    const userThreeIndex = after.findIndex((message) => {
      return message.author.id === TEST_USER_ID && message.content.includes('three')
    })
  ...
  test('normal messages bypass local queue and still show assistant text parts', async () => {
    ...
    const followupPrompt = 'Prompt from test: respond with short text for opencode queue mode.'
  ...
  test('bash tool-call actually executes and creates file in project directory', async () => {
  ...
  test('/queue shows queued status first, then dispatch indicator when dequeued', async () => {
    await discord.channel(TEXT_CHANNEL_ID).user(TEST_USER_ID).sendMessage({
    ...
    const { id: firstQueueInteractionId } = await th.user(TEST_USER_ID).runSlashCommand({
      name: 'queue',
    ...
    expect(firstQueueAckMessage.content).toContain(
  ...
  test('queue drains correctly after bursty queued messages', async () => {
    ...
    expect(textWithoutFooters).toMatchInlineSnapshot(`
        "--- from: user (queue-tester)
