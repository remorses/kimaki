...
export type ThreadStartMarker = {
  ...
  /**
   * Set on sessions spawned with `--background`: no user is ever added as a
   * thread member. Observability only — the bot does not act on it; the CLI
   * and task runner already skip the member add before posting.
   */
  background?: boolean
...
kimaki send --channel ${channelId} --prompt 'Run daily digest' --background --agent <current_agent>${parentSessionArg}
